...
public function getTrainerCourses($userId)
{
    $query = "
        ...
        INNER JOIN
            gda_courses co ON c.class_id = co.class_id
        INNER JOIN
            gda_user_class uc ON c.class_id = uc.class_id
        WHERE
            uc.user_id = :userId
    ";
    $stmt = $this->db->prepare($query);
...
public function checkAllStudentsAttended($courseId)
{
    $query = "
        ...
        FROM gda_user_class uc
        INNER JOIN gda_courses c ON uc.class_id = c.class_id
    ...
    $query = "
        SELECT COUNT(*) AS attended_students
        FROM gda_attendance
        WHERE course_id = :courseId AND (presence = 1 OR delay = 1)
    ";
    $stmt = $this->db->prepare($query);
    $stmt->bindParam(':courseId', $courseId, PDO::PARAM_INT);
    $stmt->execute();
    $attendedStudents = $stmt->fetchColumn();

    return $attendedStudents === $totalStudents;
}
}